...
class DiscoveryPipelineFC(object):
    ...
    def __init__(self, settings, spider):
        ...
        self.runtype = spider.runtype.capitalize()
    ...
    def gen_output_file_name(self):
        ...
        while os.path.exists(
                os.path.join(OUTPUT_FILE_DIR, f"{self.BOT_NAME}_{today}_{self.runtype}_{increment}.{file_extension}")):
            increment += 1

        return os.path.join(OUTPUT_FILE_DIR, f"{self.BOT_NAME}_{today}_{self.runtype}_{increment}.{file_extension}")
